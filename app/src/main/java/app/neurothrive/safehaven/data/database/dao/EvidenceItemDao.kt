...
/**
 * DAO for Evidence Items
 * Handles encrypted photos, videos, and audio recordings
 */
@Dao
interface EvidenceItemDao {
    ...
    @Query("SELECT * FROM evidence_items WHERE evidenceType = :type AND userId = :userId AND isDeleted = 0 ORDER BY timestamp DESC")
    suspend fun getByType(userId: String, type: String): List<EvidenceItem>
    ...
    @Query("SELECT COUNT(*) FROM evidence_items WHERE userId = :userId AND isDeleted = 0")
    suspend fun getCount(userId: String): Int

    @Query("UPDATE evidence_items SET syncedToSalesforce = 1, salesforceId = :salesforceId WHERE id = :id")
